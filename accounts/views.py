...
class OrganizationCreateView(LoginRequiredMixin, CreateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if not request.user.is_authenticated or request.user.role != "s_admin":
            raise PermissionDenied(
                "You do not have permission to create a user profile."
            )
        return super().dispatch(request, *args, **kwargs)
    ...
    def form_valid(self, form):
        if self.request.user.role != "s_admin":
            messages.error(
                self.request, "You don't have permission to create organization."
            )
...
class OrganizationUsersView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if search_query:
            users = (
                users.filter(username__icontains=search_query)
...
class EditOrganizationView(LoginRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        if self.request.user.role != "s_admin":
            messages.error(
                self.request, "You don't have permission to edit organization."
            )
...
class DeleteOrganizationView(LoginRequiredMixin, DeleteView):
    ...
    def form_valid(self, form):
        if self.request.user.role != "s_admin":
            messages.error(
                self.request, "You don't have permission to delete organization."
            )
...
class EditUserView(LoginRequiredMixin, UpdateView):
    ...
    def get_success_url(self):
        return reverse(
            "accounts:users", kwargs={"organization_id": self.object.organization.id}
        )
    ...
    def get_context_data(self, **kwargs):
        ...
        context["user_obj"] = (
            self.object
        )  # Using user_obj to avoid conflict with user template variable
...
def accept_patient(request, pk):
    ...
    if (
        ...
        messages.success(
            request, f"{patient_type} for {patient_name} is now in progress."
        )
    else:
