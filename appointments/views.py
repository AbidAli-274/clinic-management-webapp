...
class ReportBaseView(LoginRequiredMixin):
    ...
    def get_report_data(
        ...
        if organization_name and user.role == "s_admin":
            ...
            consultancies = consultancies.filter(patient__organization=user.organization)
...
class DailyReportView(ReportBaseView, TemplateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if not request.user.is_authenticated or request.user.role not in ["admin", "s_admin"]:
...
class DoctorReportView(LoginRequiredMixin, TemplateView):
    ...
    def categorize_feedback(self, feedback_text):
        ...
        positive_keywords = [
            ...
            "better", "improved", "helpful", "positive", "thank", "thanks"
        ]
        negative_keywords = [
            "bad", "poor", "not good", "dissatisfied", "unhappy",
            "worse", "negative", "issue", "problem", "complaint", "disappointed"
        ]
    ...
    def get_context_data(self, **kwargs):
        ...
        if selected_doctor_id:
            try:
                ...
                if total_feedback > 0:
                    satisfaction_percentage = (positive_count + (mixed_count * 0.5)) / total_feedback * 100
                else:
                ...
                for consultancy in consultancies:
                    ...
                    completion_percentage = (completed_sessions / consultancy.number_of_sessions * 100) if consultancy.number_of_sessions > 0 else 0
...
def get_session_feedback_form(request, session_id):
    ...
    if request.user.role != "room":
        return JsonResponse({"error": "Only room users can provide feedback"}, status=403)
    
    # Check if session is assigned to this room
    if session.room != request.user:
        return JsonResponse({"error": "This session is not assigned to your room"}, status=403)
    
    # Check if session is in progress
    if session.status != "Continue":
        return JsonResponse({"error": "Can only provide feedback for sessions in progress"}, status=403)
...
def submit_session_feedback(request, session_id):
    ...
    # Check if session is in progress
    if session.status != "Continue":
        return JsonResponse({"error": "Can only provide feedback for sessions in progress"}, status=400)
